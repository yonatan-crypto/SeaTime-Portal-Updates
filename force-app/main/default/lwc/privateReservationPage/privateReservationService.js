...
hideDate(){
    ...
    updateSearchDateByDay(isAddDay) {
...
formatDate(dateString) {
 const date = new Date(dateString);
    ...
    updateSelectedTypeButtonSelector(target) {
        ...
        const isCurrentlySelected = main.searchData.boatTypes.includes(selectedButtonName);

        // 1. Update the search data array
        if (isCurrentlySelected) {
            main.searchData.boatTypes = main.searchData.boatTypes.filter(type => type !== selectedButtonName);
        } else {
            main.searchData.boatTypes = [...main.searchData.boatTypes, selectedButtonName];
        }

        // 2. Update the boatTypesArray reactively so LWC updates the UI
        main.boatTypesArray = main.boatTypesArray.map(item => {
            if (item.label === selectedButtonName) {
                const newSelector = `slds-button slds-button_brand ${!isCurrentlySelected ? 'dark-button' : 'light-button'} search-button`;
                return { ...item, selector: newSelector };
            }
            return item;
        });
    }
